...
//				var items1 = query.ToArray();
		...
		public async Task FromSqlRaw()
		{
			...
			{
				var id = 1;
				var query = ctx.Categories.FromSql("SELECT * FROM [dbo].[Categories] WHERE CategoryId = {0}", id);
		...
		public async Task FromSqlInterpolated()
		{
			...
			{
				var id = 1;
				var query = ctx.Categories.FromSql($"SELECT * FROM [dbo].[Categories] WHERE CategoryId = {id}");
		...
		public void TestCommandTimeout()
		{
			...
			using (var ctx = CreateContext(false))
			{
				try
				{
					ctx.Database.ExecuteSqlCommand(createProcessLongFunctionSql);
				...
				{
					ctx.Database.ExecuteSqlCommand(dropProcessLongFunctionSql);
				}
			}
		}
